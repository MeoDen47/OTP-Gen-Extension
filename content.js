...
$(function () {
	...
	chrome.storage.sync.get('keys', function(data) {
		authenKey = data.keys;
		$('#secret').val(authenKey);
	});
	
    $(document).ready(function() {
		$("#start").click(function(){
			authenKey = $('#secret').val();
			if (authenKey != null && authenKey != ""){
				console.log("-------------start-------------");
				chrome.storage.sync.set({ 'keys': authenKey });
				updateOtp();
				timing = setInterval(timer, 1000);
			} else {
				alert("Authen Key must not null or empty\nKey is base32 format. Test key: E2TO4SSLBAFINCE");
			}
		}); 
		$("#stop").click(function(){
			console.log("-------------stop-------------")
